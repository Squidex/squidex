...
namespace Squidex.Domain.Apps.Entities.Contents.Queries.Steps
{
    public sealed class ResolveReferences : IContentEnricherStep
    {
                        ...
                        if (content.Data.TryGetValue(field.Name, out var fieldData) && fieldData != null)
                        {
                            foreach (var (partition, partitionValue) in fieldData)
                            {
                                ...
                                if (referencedContents.Count == 1)
                                {
                                    ...
                                    requestCache.AddDependency(DomainId.Combine(referencedSchema.AppId.Id.Id, referencedSchema.Id).Id, referencedSchema.Version);
                                    requestCache.AddDependency(DomainId.Combine(referencedSchema.AppId.Id.Id, referencedSchema.Id).Id, reference.Version);
        ...
        private void AddReferenceIds(HashSet<DomainId> ids, ISchemaEntity schema, IEnumerable<ContentEntity> contents)
        {
            foreach (var content in contents)
            {
