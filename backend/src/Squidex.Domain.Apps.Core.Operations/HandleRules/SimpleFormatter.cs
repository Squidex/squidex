...
public partial class SimpleFormatter(IEnumerable<IRuleEventFormatter> formatters)
{
    ...
    public async ValueTask<string> FormatAsync(string text, EnrichedEvent @event, string fallback)
    {
    ...
    private List<TextPart> BuildParts(string text, EnrichedEvent @event)
    {
        ...
        for (var i = 0; i < text.Length; i++)
        {
            ...
            if (c == '$')
            {
                ...
                var (length, part) = GetReplacement(currentSpan[(i + 1)..].ToString(), @event);
                if (length > 0)
                {
                    parts.Add(part);
                    i += length + 1;
