...
public sealed class KafkaStep : RuleFlowStep<EnrichedEvent>
{
    [LocalizedRequired]
    ...
    {
        var kafkaJob = new KafkaJob
        {
            TopicName = TopicName,
            MessageKey = Key.Or(@event.Name),
        ...
        var producer = executionContext.Resolve<KafkaProducer>();
        await producer.SendAsync(kafkaJob, ct);
