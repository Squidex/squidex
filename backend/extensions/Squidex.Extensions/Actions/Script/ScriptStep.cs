...
public class ScriptStep : RuleFlowStep<EnrichedEvent>
{
    [LocalizedRequired]
        ...
        CancellationToken ct)
    {
        ...
        if (@event is EnrichedUserEventBase)
        {
            vars.User = AllPrinicpal();
        }

        var scriptEngine = executionContext.Resolve<IScriptEngine>();

        var result = await scriptEngine.ExecuteAsync(vars, Script, ct: ct);
        executionContext.Log("Script executed", result);
