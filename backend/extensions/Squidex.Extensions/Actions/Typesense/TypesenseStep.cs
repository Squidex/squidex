...
public sealed class TypesenseStep : RuleFlowStep<EnrichedEvent>
{
    [AbsoluteUrl]
    [LocalizedRequired]
    ...
    {
        var url = $"{Host.ToString().TrimEnd('/')}/collections/{IndexName}/documents";
        ...
        var httpClient = executionContext.Resolve<IHttpClientFactory>().CreateClient("TypesenseAction");

        var delete = executionContext.ShouldDelete(context, Delete);
        if (delete)
        {
            await DeleteAsync(executionContext, contentId, url, httpClient, ct);
        }
        else
        {
            await UpsertAsync(executionContext, url, httpClient, ct);
        }

        return FlowStepResult.Next();
    }

    private async Task DeleteAsync(FlowExecutionContext executionContext, string contentId, string url, HttpClient client,
        CancellationToken ct)
    {
        var request = new HttpRequestMessage(HttpMethod.Delete, $"{url}/{contentId}");

        await SendAsync(executionContext, client, request, Document, ct);

        executionContext.Log($"Deleted entry from Typesense index: {IndexName}");
    }

    private async Task UpsertAsync(FlowExecutionContext executionContext, string url, HttpClient client,
        CancellationToken ct)
    {
        var request = new HttpRequestMessage(HttpMethod.Post, $"{url}?action=upsert")
        {
        ...
        executionContext.Log($"Added entry to Typesense index: {IndexName}");
    }

    private async Task SendAsync(FlowExecutionContext executionContext, HttpClient client, HttpRequestMessage request, string? body,
        CancellationToken ct)
    {
        request.Headers.TryAddWithoutValidation("X-Typesense-Api-Key", ApiKey);

        await client.OneWayRequestAsync(request, executionContext.Log, body, ct);
    }
}
