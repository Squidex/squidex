...
public sealed class CreateContentStep : RuleFlowStep<EnrichedEvent>
{
    [LocalizedRequired]
    ...
    protected override async ValueTask PrepareAsync(RuleFlowContext context, EnrichedEvent @event, FlowExecutionContext executionContext,
        CancellationToken ct)
    {
        var appProvider = executionContext.Resolve<IAppProvider>();

        var schema = await appProvider.GetSchemaAsync(@event.AppId.Id, Schema, true, ct)
            ?? throw new InvalidOperationException($"Cannot find schema '{Schema}'");
        ...
        CancellationToken ct)
    {
        ...
        var actor = RefToken.Client("Rules");
        ...
        var command = new Command
        {
            AppId = @event.AppId,
            Actor = actor,
            Data = jsonSerializer.Deserialize<ContentData>(Data),
            SchemaId = SchemaId,
            Status = status,
            FromRule = true,
        };
