...
public sealed class FastlyStep : RuleFlowStep<EnrichedEvent>
{
    [LocalizedRequired]
        ...
        CancellationToken ct)
    {
        ...
        {
            key = DomainId.Combine(@event.AppId.Id, entityEvent.Id).ToString();
        }

        var httpClient = executionContext.Resolve<IHttpClientFactory>().CreateClient("FastlyAction");

        var requestUrl = $"/service/{ServiceId}/purge/{key}";
        var request = new HttpRequestMessage(HttpMethod.Post, requestUrl);

        request.Headers.Add("Fastly-Key", ApiKey);

        await httpClient.OneWayRequestAsync(request, executionContext.Log, ct: ct);
