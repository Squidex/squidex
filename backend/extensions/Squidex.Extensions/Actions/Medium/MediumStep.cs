...
public sealed class MediumStep : RuleFlowStep<EnrichedEvent>
{
    [LocalizedRequired]
    ...
    {
        var httpClient = executionContext.Resolve<IHttpClientFactory>().CreateClient("MediumAction");
        ...
        else
        {
            ...
            catch (Exception ex)
            {
                ...
                executionContext.Log("Failed to fetch path", requestDump);
                throw;
            }
        }
        ...
        await httpClient.OneWayRequestAsync(BuildPostRequest(path, jsonBody), executionContext.Log, jsonBody, ct);
