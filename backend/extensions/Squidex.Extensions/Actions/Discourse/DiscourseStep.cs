...
public sealed class DiscourseStep : RuleFlowStep<EnrichedEvent>
{
    [AbsoluteUrl]
    [LocalizedRequired]
        ...
        CancellationToken ct)
    {
        ...
        if (Topic != null)
        {
            json.Add("topic_id", Topic.Value);
        }

        if (Category != null)
        {
            json.Add("category", Category.Value);
        }

        var jsonSerializer = executionContext.Resolve<IJsonSerializer>();
        var jsonRequest = jsonSerializer.Serialize(json);

        var httpClient = executionContext.Resolve<IHttpClientFactory>().CreateClient("DiscourseAction");
        ...
        await httpClient.OneWayRequestAsync(request, executionContext.Log, jsonRequest, ct);
