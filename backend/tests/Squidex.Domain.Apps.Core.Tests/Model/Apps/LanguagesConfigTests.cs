...
namespace Squidex.Domain.Apps.Core.Model.Apps
{
    ...
    {
        private readonly LanguagesConfig config_0 = LanguagesConfig.Build(Language.DE);
        ...
        public void Should_create_initial_config_0_with_multiple_languages()
        {
            ...
            config.OfType<LanguageConfig>().ToList().Should().BeEquivalentTo(
                new List<LanguageConfig>
                {
                    new LanguageConfig(Language.DE),
            ...
            config.TryGetConfig(Language.DE, out var master);

            Assert.Equal(Language.DE, config.Master.Language);
            Assert.Equal(3, config.Count);

            Assert.Same(master, config.Master);
        }
        ...
        public void Should_create_initial_config_0_with_config_0s()
        {
            ...
            config.OfType<LanguageConfig>().ToList().Should().BeEquivalentTo(configs);

            Assert.Equal(configs[0], config.Master);
            Assert.Same(configs[0], config.Master);
        }
        ...
        public void Should_make_master_language()
        {
            ...
            var config_2 = config_1.Set(Language.IT, true, new[] { Language.UK });
            var config_3 = config_2.MakeMaster(Language.IT);

            Assert.Equal(Language.IT, config_3.Master.Language);

            Assert.False(config_3.Master.IsOptional);
        }
        ...
        public void Should_remove_language()
        {
            ...
            var config_1 = config.Remove(Language.IT);
