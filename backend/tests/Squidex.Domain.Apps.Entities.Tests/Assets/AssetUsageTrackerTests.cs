...
//        {
//            A.CallTo(() => appUsageGate.GetAsync($"{appId.Id}_Assets", default, default, null, default))
...
//            A.CallTo(() => appUsageGate.QueryAsync($"{appId.Id}_Assets", dateFrom, dateTo, default))
...
//            A.CallTo(() => appUsageGate.TrackAssetAsync(appId,, null, A<Counters>._, default))
//                .Invokes(x => countersSummary = x.GetArgument<Counters>(3));

//            A.CallTo(() => appUsageGate.TrackAsync(date, $"{appId.Id}_Assets", null, A<Counters>._, default))
