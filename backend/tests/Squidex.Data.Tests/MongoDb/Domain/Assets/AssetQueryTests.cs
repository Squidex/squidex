...
public class AssetQueryTests
{
    ...
    {
        var filter = query.AdjustToAssetModel(appId).BuildFilter<MongoAssetEntity>(false).Filter!;
    ...
    private void AssertSorting(string expected, params SortNode[] sort)
    {
        ...
        cursor.QuerySort(new ClrQuery { Sort = sort.ToList() }.AdjustToAssetModel(appId));
