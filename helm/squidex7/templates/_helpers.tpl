...
{{- define "squidex.mongoDefaultHostname" }}
  ...
  {{- range $index, $i := until (int (index .Values "mongodb-replicaset").replicaCount) }}
    {{- $replica := printf "%s-mongodb-replicaset-%d.%s-mongodb-replicaset-headless.%s.svc.%s" $release.Name $i $release.Name $release.Namespace $clusterSuffix }}
    {{- if eq $i 0}}
      {{- $replica }}
    {{- else -}}
      ,{{ $replica }}
