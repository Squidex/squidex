...
CMD ["bash", "-c", "echo .NET VERSION && dotnet --version && echo GLIBC VERSION && ldd --version && echo STARTING BACKEND TESTS && cd backend && dotnet test --filter \"Category!=Dependencies & Category!=TestContainer\" --configuration Release --logger trx --collect:\"XPlat Code Coverage\""]
