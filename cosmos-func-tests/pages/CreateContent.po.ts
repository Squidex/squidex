...
export class CreateContent extends BrowserUtil {
    ...
    public async selectContent(content: string) {
        ...
        if (searchResult.isPresent() && await searchResult.getText() === content) {
    ...
    public getCommentaryEditor() {
        return element(by.xpath('//iframe[@src=\'http://localhost:5000/editors/toastui/md-editor.html\']'));
    }

    public getCommentaryEditorInput() {
        return element(by.xpath('//div[@contenteditable=\'true\']'));
    }

    public async getCommentary() {
